...
func (f *fileInputReader) parse(init chan struct{}) error {
	...
	for {
		...
		if bytes.Equal(payloadSeparatorAsBytes[1:], line) {
			...
			if len(meta) < 3 {
				glogs.Debug(1, fmt.Sprintf("Found malformed record, file: %s, line %d", f.path, lineNum))
				buffer = bytes.Buffer{}
				continue
			}
...
func newFileInputReader(path string, readDepth int, dryRun bool) *fileInputReader {
	...
	if err != nil {
		glogs.Debug(0, fmt.Sprintf("[INPUT-FILE] err: %q", err))
		return nil
	}
...
func (i *FileInput) init() (err error) {
	...
	if matches, err = filepath.Glob(i.path); err != nil {
		glogs.Debug(2, "[INPUT-FILE] Wrong file pattern", i.path, err)
		return
	}

	if len(matches) == 0 {
		glogs.Debug(2, "[INPUT-FILE] No files match pattern: ", i.path)
...
func (i *FileInput) PluginRead() (*common.Message, error) {
...
func (i *FileInput) emit() {
	...
	glogs.Debug(2, fmt.Sprintf("[INPUT-FILE] FileInput: end of file '%s'\n", i.path))
