...
type PcapOptions struct {
	...
	BufferSize      common.Size     `json:"input-raw-buffer-size"`
...
func NewListener(host string, ports []uint16, config PcapOptions) (l *Listener, err error) {
	...
	l.messages = make(chan *tcp.TcpMessage, 10000)
...
func (l *Listener) PcapHandle(ifi pcap.Interface) (handle *pcap.Handle, err error) {
	...
	bpfFilter = "tcp dst port 20881"

	//bpfFilter = "((tcp dst port 20881) and (dst host ::1 or dst host 127.0.0.1))"
...
func http1EndHint(m *tcp.TcpMessage) bool {
	if m.MissingChunk() {
		return false
	}
...
func (l *Listener) Messages() chan *tcp.TcpMessage {
	return l.messages
}
